...
impl HostAlignedByteCount {
    ...
    pub fn checked_sub(self, bytes: HostAlignedByteCount) -> Result<Self, ByteCountOutOfBounds> {
        // aligned - aligned = aligned
        self.0.checked_sub(bytes.0).map(Self).ok_or_else(|| {
            ByteCountOutOfBounds(ByteCountOutOfBoundsKind::Sub {
                minuend: self,
                subtrahend: bytes,
            })
        })
    ...
    pub fn checked_mul(self, scalar: usize) -> Result<Self, ByteCountOutOfBounds> {
        ...
        self.0.checked_mul(scalar).map(Self).ok_or_else(|| {
            ByteCountOutOfBounds(ByteCountOutOfBoundsKind::Mul {
                byte_count: self,
                scalar,
            })
        })
...
enum ByteCountOutOfBoundsKind {
    RoundUp(usize),
    U64(u64),
    Add(HostAlignedByteCount, HostAlignedByteCount),
    Sub {
        // minuend - subtrahend
        minuend: HostAlignedByteCount,
        subtrahend: HostAlignedByteCount,
    },
    Mul {
        byte_count: HostAlignedByteCount,
        scalar: usize,
    },
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ByteCountOutOfBoundsKind::RoundUp(bytes) => {
                write!(
                    f,
                    "byte count overflow rounding up: {}",
                    LowerHexDisplay(*bytes)
                )
            }
            ByteCountOutOfBoundsKind::U64(bytes) => {
                write!(
                    f,
                    "byte count overflow converting u64: {}",
                    LowerHexDisplay(*bytes)
                )
            }
            ...
            ByteCountOutOfBoundsKind::Sub {
                minuend,
                subtrahend,
            } => {
                write!(f, "byte count underflow computing {minuend} - {subtrahend}")
            }
            ByteCountOutOfBoundsKind::Mul { byte_count, scalar } => {
                write!(
                    f,
                    "byte count overflow multiplying {byte_count} by {scalar}"
                )
            }
        }
    }
...
mod tests {
    ...
    fn byte_count_display() {
        ...
        assert_eq!(format!("{:x}", byte_count), "10000");
        assert_eq!(format!("{:#x}", byte_count), "0x10000");
    }
